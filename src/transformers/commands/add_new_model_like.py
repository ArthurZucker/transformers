...
MODULAR_HEADER_TEMPLATE = (
    "# This file was generated by `transformers add-new-model-like` as a modular implementation.\n"
    "# After adapting it, convert to the single-file format with:\n"
    "#   python utils/modular_model_converter.py --files_to_parse src/transformers/models/{name}/modular_{name}.py\n"
)
...
def create_new_model_like(
    ...
    Args:
        ...
        directory_exists (`bool`, *optional*, defaults to `False`):
            If `True`, skip file creation and only update the AUTO mappings.
    """
    ...
    if directory_exists and module_folder.exists():
        add_content_to_file(
    ...
    for module_file in files_to_adapt:
        ...
        header = None
        is_modeling_file = "modeling" in module_file.name
        if is_modeling_file:
            new_module_name = re.sub(r"modeling(_(tf|flax))?_", "modular_", new_module_name)
            header = MODULAR_HEADER_TEMPLATE.format(name=new_model_patterns.model_lower_cased)
        ...
        duplicate_module(
            ...
            new_model_patterns,
            dest_file=dest_file,
            add_copied_from=add_copied_from and is_modeling_file,
            header=header,
        )
...
class AddNewModelLikeCommand(BaseTransformersCLICommand):
    ...
    def run(self):
        ...
        create_new_model_like(
            ...
            directory_exists=(TRANSFORMERS_PATH / "models" / self.model_patterns.model_lower_cased).exists(),
        )
